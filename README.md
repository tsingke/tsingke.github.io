...
| 目录 | 模块 | 说明 |
|---|---|---|
| `_pages/` | 页面 | `home.md` 首页 · `publications.html` 论文 · `teaching.html` 教学 · `lab.md` CILab(含团队成员) · `recruiting.md` 招生 · `cv.md` 简历 · `contact.html` 联系 |
...
| 改团队成员名单 | 编辑 `_data/members.yml`（在读/毕业生、本科生三组，展示于 Lab 页） |
...
- **中英文切换**：导航按钮即时切换界面文案与正文（双语块），记忆偏好（localStorage）
- **黑白模式**：月亮/太阳按钮手动切换，默认跟随系统，记忆偏好
- **论文模块**：按引用数排序精选，PDF/源代码/在线阅读/引用四按钮，BibTeX 弹窗，详情页含摘要/关键词/截图
- **课程资料站**：每门课含中英课程介绍、教材、授课目录、章节课件、课程代码、视频、参考资料
- **留言与项目合作**：Contact 页邮件表单（mailto 方案，类型可选）
- **CILab 实验室**：团队（在读研究生点击详情、毕业生、本科生三组）、研究领域配图、项目、专利、实验室地址
- **招生模块**：招生流程流线、培养理念、研究方向、学生发展、软硬件资源、导师寄语
- **动画**：导航下划线、卡片 hover、滚动显现、招生流线（尊重 prefers-reduced-motion）
